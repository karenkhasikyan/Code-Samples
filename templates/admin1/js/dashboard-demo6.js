var demo = {
  init: function(){
    ...
    // Tabdrop plugin
    // Function Arguments: selector
    demo.tabdropInit('.dashboard-tabs');
    $('.dashboard-tabs a').on('shown.bs.tab', $.proxy(demo.tabdropInit, null, '.panel-heading .nav-tabs'));
    ...
    // Vector map
    // Function Arguments: selector
    $('a[href="#ecommerce-dashboard"]').on('shown.bs.tab', $.proxy(demo.mapInit, null, '#map'));
    ...
    $('a[href="#ecommerce-orders"]').on('shown.bs.tab', $.proxy(demo.icheckInit, null, '#ecommerce-orders input.icheck'));

    $('a[href="#ecommerce-orders"]').on('shown.bs.tab', $.proxy(demo.datatablesIndividualSearchInit, null, '#ecommerce-orders .table-ecommerce'));


    // Products tab
    // Function Argument: selector
    $('a[href="#ecommerce-products"]').on('shown.bs.tab', $.proxy(demo.icheckInit, null, '#ecommerce-products input.icheck'));
    
    $('a[href="#ecommerce-products"]').on('shown.bs.tab', $.proxy(demo.datatablesIndividualSearchInit, null, '#ecommerce-products .table-ecommerce'));
  },
  ...
  datatablesTasklistInit: function(el){
    $(el).DataTable({
      ...
      initComplete: function(settings, json){
        ...
        demo.icheckInit(el+' input.icheck');

        // If checkbox in <th> is checked, marking checked all checkboxes too
        $(el+' .icheck').on('ifChecked', demo.datatablesIcheckChecked);

        // If checkbox in <th> is unchecked, marking unchecked all checkboxes too
        $(el+' .icheck').on('ifUnchecked', demo.datatablesIcheckUnchecked);
  ...
  datatablesVisitorsActivityInit: function(el){
    $(el).dataTable({
      ...
      initComplete: function(settings, json){
        ...
        var searchInput = panel.find('.dataTables_filter').html(panel.find('.dataTables_filter .form-control'));
  ...
  datatablesIndividualSearchInit: function(el){
    if( ! $.fn.DataTable.isDataTable(el) ){
      // Datatables
      // Initializing datatables if it doesn't exist
