var wisemode = {
  ...
  rowColObj: {},
    ...
    var device = 'ontouchstart' in document.documentElement;
      ...
      $('.breadcrumb-wrapper select').remove();
    }
	},
  sortableInit: function(){
    ...
    $($rowSortable).sortable({
      ...
      update: function(event, ui) {
        ...
        if( wisemode.select2Val('.breadcrumb-wrapper select') == 'on' ){
    ...
    if( $('.breadcrumb-wrapper select').length > 0 ){
      if( wisemode.select2Val('.breadcrumb-wrapper select') == 'on' ){
        wisemode.dataStore('on');
      }

      $('.breadcrumb-wrapper select').on('change', wisemode.select2ValOnChange);
    }
  },
